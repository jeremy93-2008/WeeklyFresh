...
const EMPTY_INGREDIENT: IngredientRow = { quantity: "", unit: "", name: "", shipped: true };
const EMPTY_INSTRUCTION: InstructionRow = { text: "", image: null };
...
export function useRecipeForm(initialData?: RecipeFormData) {
  ...
  const [instructions, setInstructions] = useState<InstructionRow[]>(
    initialData?.instructions?.length ? initialData.instructions : [EMPTY_INSTRUCTION]
  );
  ...
  function validate(): string | null {
    ...
    if (validIngredients.length === 0) return "Agrega al menos un ingrediente";
    return null;
  }
  ...
  return {
    ...
    ingredients, instructions, utensils,
    isPending, isEditing,

    // Derived
    validIngredients, validInstructions, validUtensils,

    // Ingredient actions
    updateIngredient, addIngredient, removeIngredient,

    // Instruction actions
    updateInstruction, addInstruction, removeInstruction, moveInstruction,

    // Utensil actions
    updateUtensil, addUtensil, removeUtensil,
